...
s'il s'agit d'une erreur honnête). MKEcoin est confidentiel, de sorte que vous ne pouvez pas
...
transaction à un tiers, celui-ci peut dire si la transaction à envoyé des MKEcoinj à cette adresse
...
des MKEcoinj à une adresse), vous pouvez le vérifier comme suit :

    check_tx_key IDTX CLEFTX ADRESSE

Remplacez `IDTX`, `CLEFTX` et `ADRESSE` respectivement par l'ID de transaction, la clef propre
à la transaction et l'adresse de destination qui vous ont été communiqués. MKEcoin-wallet-cli va
vérifier cette transaction et vous indiquera combien de MKEcoinj ont été payés à l'adresse fournie.
