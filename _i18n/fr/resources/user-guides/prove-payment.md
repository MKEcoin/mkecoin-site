...
MKEcoin cependant est confidentiel : cette information n'est pas disponible publiquement sur la chaîne de blocs. Les étapes sont donc
...
- La clef de la transaction, qui est une nouveauté de MKEcoin et d'autres monnaies CryptoNote.
...
Alice peut la demander dans MKEcoin-wallet-cli (nouveau nom de l'ancien simplewallet) :
...
Notez que cela ne fonctionnera que si MKEcoin-wallet-cli est configuré pour sauvegarder les clefs de transaction. Pour le vérifier :
...
Alice peut ouvrir son MKEcoin-wallet-gui et aller à la page historique pour voir les détails de la transaction :
...
Charlie entre dans MKEcoin-wallet-cli :

> check_tx_key IDTX CLEFTX ADRESSE

Les données fournies par Alice entrées en lieux et place de leurs identifiants respectifs. MKEcoin-wallet-clu va se servir
...
Charlie ouvre son MKEcoin-wallet-gui et va à la page Avancé > Prouver/vérifier afin de remplir la section Vérifier avec les éléments transmis par Alice :
