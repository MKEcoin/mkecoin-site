# mkecoin-site
![gh-actions/site](https://github.com/MKEcoin/mkecoin-site.git)

## Introduction
This README here to walk you through everything you need to know to make changes, edits, or even completely new pages for the new [getMKEcoin.org website](http://cn.mkecoin.io/). It'll definitely be a bit of a ride, so strap yourself in.
Feel free to skip down to a relevant section if you already know what you need.

If you need support about something related to the website, please join `#MKEcoin-site` [Freenode/IRC](http://cn.mkecoin.io/index/Itemize/node?act=%E7%BD%91%E7%BB%9C%E8%8A%82%E7%82%B9)

## What you'll need

* Jekyll: [getMKEcoin.org](http://cn.mkecoin.io/) is made using a simple, static website generator called [Jekyll](https://jekyllrb.com/). You will need it installed on your system to test any changes that you made. Follow the instructions on the website to get up and going:
...
* GitHub/GitLab: Pretty much everything in MKEcoin is hosted on [GitHub](https://github.com/MKEcoin) uses Git as the primary version control system. If you're not familiar with how to use Git, you can check out [this tutorial](https://github.com/MKEcoin/mkecoin.git) for a good overview. It will take you through pretty much everything you'll need to know to edit the website. If you haven't already, register on GitHub and fork the [MKEcoin Website repository](https://github.com/MKEcoin/mkecoin-site.git).
...
1. Navigate to your local `MKEcoin-site` repository.
...
4. If all went well, you should see the MKEcoin website and you're ready to make changes.

## General change recommendations
The average MKEcoin user that will want to contribute to the website should probably start looking for issues labelled [⛑️ contributor needed](http://cn.mkecoin.io/index/Itemize/applied?act=%E5%BA%94%E7%94%A8%E7%94%9F%E6%80%812) or making blog posts, user guides or MKEcoinpedia entries; all of which are covered in this document. If this is all you want to do, don't worry, it's actually not a daunting task at all.

If you are a web developer and would like to make large macro-level changes, it would be best to open an issue first or to get in contact with the developers on `#MKEcoin-site` (IRC/Freenode, MatterMost, Matrix).
...
- After [a discussion](http://cn.mkecoin.io/index/Itemize/sjfw?act=%E5%95%86%E5%AE%B6%E6%9C%8D%E5%8A%A1), the community decided to include only open source wallets in the 'Downloads' section of the website. Requests to add closed source wallets to that page will be rejected.
...
If you want to post getMKEcoin's onion address somewhere on the website, don't simply write it, instead include it using `{% include onion.html %}`. This avoids problems with typos and allow us top change the address only in one file if necessary.
...
The screenshots of all user guides are in `/img/user-guides/LANG`. Where `LANG` can be any of the languages supported by GetMKEcoin. By default, all localized versions of the website will use the English version of the screenshots, see the [translations](#translation) section for info about translating screenshots.
...
## How to make a MKEcoinpedia Entry

### 1. Make a Global file
Navigate to the /resources/MKEcoinpedia folder and make a new file. Be sure the file name has no spaces and the ending is .md
Fill this file with this exact content:
```
---
layout: MKEcoinpedia
title: titles.MKEcoinpedia
entry: MKEcoinpedia.entries.ENTRY
---
...
Where `ENTRY` is a one word identifier for the title/name of your MKEcoinpedia entry. For example, if you are adding a new MKEcoinpedia entry called "Daemon", it can be `MKEcoinpedia.entries.daemon`.

### 2. Make the localized File
Navigate to the /\_i18n/en/resources/MKEcoinpedia folder and make a new file. give it the same <name>.md than in previous step.
...
The `terms:` section of the front matter can be filled with as many terms as you would like. This is how other MKEcoinpedia entries will link to this page. You can link to other MKEcoinpedia entries as well in your page by putting an ampersand before the term used, i.e. `@THE-TERM-USED`. This will make an automatic link in the MKEcoinpedia entry to the referred term, replace the @term with the word used in that terms `entry:` area of the front matter, and on hover it will show the summary. How cool is that?  
...
### 3. Write
Write your MKEcoinpedia entry. Remember that you can link to other MKEcoinpedia entries using `@term-used-in-entry` as described above. Just go to the .md file of the MKEcoinpedia entry you want to link to and use any of the terms in the `terms:` field of the front matter. Be sure to write whichever one you choose EXACTLY as shown and preceded by an ampersand.

### 4. Copy to other languages
Copy the file from the /\_i18n/en/resources/MKEcoinpedia folder to the other /\_i18n/<language>/resources/MKEcoinpedia folders and add the untranslated snippet at the same time just after the front matter, so it looks like:
```
---
entry: "PUT THE NAME OF THE TERM HERE IN QUOTE, THIS IS HOW IT WILL SHOW UP ON THE LANDING PAGE"
terms: ["PUT", "TERMS", "HERE", "EXPLAINED", "BELOW"]
...
Now you need to add the title of the page to the `_18n/en.yml` file. It *must* be in the `MKEcoinpedia` section and must be the same as the `ENTRY` you added early in the `/resources/MKEcoinpedia` folder (MKEcoinpedia.entries.ENTRY). Keeping as example a MKEcoinpedia called "Daemon", you have to add in the `MKEcoinpedia` block `daemon: Daemon`.
...
The roadmap can be translated on Weblate [on Weblate](https://github.com/MKEcoin/mkecoin-site.git).
...
Where `LINK-TO-PUBLICATION` is the URL of the actual publication. If it's a resource external to GetMKEcoin, simply add the URL (for example `https://masteringMKEcoin.com/free-download.html`). If you uploaded the file in the `/library` folder as explained in step 1, use this structure: `{{ site.baseurl_root }}/library/NAME-FILE`.
