...
This is a list of the MKEcoin-wallet-rpc calls, their inputs and outputs, and examples of each. The program MKEcoin-wallet-rpc replaced the rpc interface that was in simplewallet and then MKEcoin-wallet-cli.

All MKEcoin-wallet-rpc methods use the same JSON RPC interface. For example:
...
If the MKEcoin-wallet-rpc was executed with the `--rpc-login` argument as `username:password`, then follow this example:
...
Note: "@atomic-units" refer to the smallest fraction of 1 XMR according to the MKEcoind implementation. **1 XMR = 1e12 @atomic-units.**
...
Connect the RPC server to a MKEcoin daemon.
...
* *balance* - unsigned int; The total balance of the current MKEcoin-wallet-rpc in session.
* *unlocked_balance* - unsigned int; Unlocked funds are those funds that are sufficiently deep enough in the MKEcoin blockchain to be considered safe to spend.
...
* *address* - string; The 95-character hex address string of the MKEcoin-wallet-rpc in session.
...
Analyzes a string to determine whether it is a valid MKEcoin wallet address and returns the result and the address specifications.

Inputs:

* *address* - string; The address to validate.
* *any_net_type* - boolean (Optional); If true, consider addresses belonging to any of the three MKEcoin networks (mainnet, stagenet, and testnet) valid. Otherwise, only consider an address valid if it belongs to the network on which the rpc-wallet's current daemon is running (Defaults to false).
* *allow_openalias* - boolean (Optional); If true, consider [OpenAlias-formatted addresses]({{ site.baseurl }}/resources/MKEcoinpedia/openalias.html) valid (Defaults to false).

Outputs:
* *valid* - boolean; True if the input address is a valid MKEcoin address.
* *integrated* - boolean; True if the given address is an [integrated address]({{ site.baseurl }}/resources/MKEcoinpedia/address.html).
* *subaddress* - boolean; True if the given address is a [subaddress](https://github.com/MKEcoin-project/MKEcoin/pull/2056)
* *nettype* - string; Specifies which of the three MKEcoin networks (mainnet, stagenet, and testnet) the address belongs to.
* *openalias_address* - boolean; True if the address is [OpenAlias-formatted]({{ site.baseurl }}/resources/MKEcoinpedia/openalias.html).
...
* *height* - unsigned int; The current MKEcoin-wallet-rpc's blockchain height. If the wallet has been offline for a long time, it may need to catch up with the daemon.
...
Send MKEcoin to a number of recipients.
...
* *unlock_time* - unsigned int; Number of blocks before the MKEcoin can be spent (0 to not add a lock).
* *get_tx_key* - boolean; (Optional) Return the transaction key after sending.
* *do_not_relay* - boolean; (Optional) If true, the newly created transaction will not be relayed to the MKEcoin network. (Defaults to false)
...
* *desc* - The description of the transfer as a list of:
  ...
  * *unlock_time* - unsigned int; The number of blocks before the MKEcoin can be spent (0 for no lock).
...
$ curl http://127.0.0.1:18082/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"make_uri","params":{"address":"55LTR8KniP4LQGJSPtbYDacR7dz8RBFnsfAKMaMuwUNYX6aQbBcovzDPyrQF9KXF9tVU6Xk3K8no1BywnJX6GvZX8yJsXvt","amount":10,"payment_id":"420fa29b2d9a49f5","tx_description":"Testing out the make_uri function.","recipient_name":"el00ruobuob Stagenet wallet"}}'  -H 'Content-Type: application/json'
{
  ...
  "jsonrpc": "2.0",
  "result": {
    "uri": "MKEcoin:55LTR8KniP4LQGJSPtbYDacR7dz8RBFnsfAKMaMuwUNYX6aQbBcovzDPyrQF9KXF9tVU6Xk3K8no1BywnJX6GvZX8yJsXvt?tx_payment_id=420fa29b2d9a49f5&tx_amount=0.000000000010&recipient_name=el00ruobuob%20Stagenet%20wallet&tx_description=Testing%20out%20the%20make_uri%20function."
  }
}
```
...
Start mining in the MKEcoin daemon.
...
Stop mining in the MKEcoin daemon.
...
Create a new wallet. You need to have set the argument "--wallet-dir" when launching MKEcoin-wallet-rpc to make this work.
...
Open a wallet. You need to have set the argument "--wallet-dir" when launching MKEcoin-wallet-rpc to make this work.
