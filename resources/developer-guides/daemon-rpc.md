...
This is a list of the MKEcoind daemon RPC calls, their inputs and outputs, and examples of each.
...
The majority of MKEcoind RPC calls use the daemon's `json_rpc` interface to request various bits of information. These methods all follow a similar structure, for example:
...
* *block_header* - A structure containing block header information.
  ...
  * *difficulty* - unsigned int; The strength of the MKEcoin network based on mining power.
  * *hash* - string; The hash of this block.
  * *height* - unsigned int; The number of blocks preceding this block on the blockchain.
  * *major_version* - unsigned int; The major version of the MKEcoin protocol at this block height.
  * *minor_version* - unsigned int; The minor version of the MKEcoin protocol at this block height.
  * *nonce* - unsigned int; a cryptographic random one-time number used in mining a MKEcoin block.
...
For calls that end with **.bin**, the data is exchanged in the form of binary, serialized objects, as defined in the [Core RPC Server](https://github.com/MKEcoin-project/MKEcoin/blob/master/src/rpc/core_rpc_server_commands_defs.h).
...
* *gray_list* - array of offline *peer* structure as follows:
  ...
  * *port* - unsigned int; TCP port the peer is using to connect to MKEcoin network.
* *status* - string; General RPC error code. "OK" means everything looks good. Any other value means that something went wrong.
